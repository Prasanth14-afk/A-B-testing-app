'use client'

import { motion } from 'framer-motion'
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts'

interface RetentionBarChartProps {
  data: Array<{
    metric: string
    gate30: number
    gate40: number
    significant?: boolean
  }>
}

const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-card border border-border rounded-xl p-3 shadow-lg">
        <p className="text-sm font-semibold text-foreground mb-2">{payload[0].payload.metric}</p>
        {payload.map((entry: any, index: number) => (
          <div key={index} className="flex items-center gap-2 text-xs">
            <div className="w-3 h-3 rounded" style={{ backgroundColor: entry.color }}></div>
            <span className="text-muted">{entry.name}:</span>
            <span className="font-semibold text-foreground">{entry.value.toFixed(1)}%</span>
          </div>
        ))}
        {payload[0].payload.significant && (
          <div className="mt-2 px-2 py-1 bg-positive/10 text-positive text-xs rounded font-medium">
            ✓ Significant (p &lt; 0.05)
          </div>
        )}
      </div>
    )
  }
  return null
}

export default function RetentionBarChart({ data }: RetentionBarChartProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: 0.2 }}
      className="bg-card rounded-2xl p-6 card-shadow"
    >
      <div className="mb-6">
        <h3 className="text-lg font-bold text-foreground">Retention Comparison</h3>
        <p className="text-sm text-muted mt-1">Day 1 vs Day 7 retention rates by gate</p>
      </div>

      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
          <XAxis 
            dataKey="metric" 
            stroke="hsl(var(--muted))"
            tick={{ fill: 'hsl(var(--muted))', fontSize: 12 }}
            axisLine={{ stroke: 'hsl(var(--border))' }}
          />
          <YAxis 
            stroke="hsl(var(--muted))"
            tick={{ fill: 'hsl(var(--muted))', fontSize: 12 }}
            axisLine={{ stroke: 'hsl(var(--border))' }}
            label={{ value: 'Retention %', angle: -90, position: 'insideLeft', fill: 'hsl(var(--muted))' }}
          />
          <Tooltip content={<CustomTooltip />} cursor={{ fill: 'hsl(var(--border))' }} />
          <Legend 
            wrapperStyle={{ paddingTop: '20px' }}
            iconType="circle"
            formatter={(value) => <span className="text-sm text-muted">{value}</span>}
          />
          <Bar 
            dataKey="gate30" 
            name="Gate 30" 
            fill="hsl(var(--brand))" 
            radius={[8, 8, 0, 0]}
            maxBarSize={60}
          />
          <Bar 
            dataKey="gate40" 
            name="Gate 40" 
            fill="hsl(var(--accent))" 
            radius={[8, 8, 0, 0]}
            maxBarSize={60}
          />
        </BarChart>
      </ResponsiveContainer>
    </motion.div>
  )
}
